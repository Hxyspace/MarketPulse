...
| **招商银行汇率** | USD/HKD 现汇买入、现汇卖出 | 实时牌价 + 历史走势 | 招商银行外汇牌价 |

## 📸 截图

### Web 面板

> 访问 `http://localhost:3000` 查看市场分析；访问 `http://localhost:3000/fx` 查看招商银行汇率。均支持亮/暗主题切换。
...
每天 22:00 额外推送 **招商银行汇率图** — 美元 / 港币实时牌价 + 历史现汇买入卖出走势。
...
配置完成后，每个交易日 09:00（北京时间）自动推送市场报告，每天 22:00 推送招商银行汇率图。卡片内含「查看面板」按钮
...
├── modules/
│   └── fx/
│       ├── routes.ts         # /api/fx/* 汇率 API
│       └── services/
│           ├── cmbFx.ts      # 招商银行汇率
│           └── cmbFxImage.ts # 招商银行汇率图片
├── calculators/
...
| 招商银行汇率 | 招商银行外汇牌价 API | USD/HKD 实时牌价与历史现汇买入卖出 |

**更新策略**：北京时间 18:00 前取前一交易日，之后取当天。每个交易日只拉取一次，增量更新。

> 招商银行历史汇率按北京 22:00 前后取值更新。
